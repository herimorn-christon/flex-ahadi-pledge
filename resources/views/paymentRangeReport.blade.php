...
<title>payment date range report</title>
...
//user=Auth::User()->id;
 ...
 $money_pledge=App\Models\payment::where('user_id',$user) ->whereBetween('created_at', [$fromDate, $toDate])->
 sum('amount');
 $ahadi=App\Models\Pledge::where('user_id',$user)->
 whereBetween('created_at', [$fromDate, $toDate])
 ->sum('amount');
 $total=App\Models\Payment::select(['amount'])
                    ->where('user_id',$user)
                    ->whereBetween('created_at', [$fromDate, $toDate])
                    ->sum('amount');
 $payments =App\Models\Payment::select(['user_id', 'pledge_id','type_id','amount','created_at'])
                        ->whereBetween('created_at', [$fromDate, $toDate])
                        ->where('user_id',$user)
                        ->with('payer')
                        ->with('payment')
    ...
    <div class="my-5 " size="A4" style="margin-left:-9.4%;position:relative;top:-10%">
        <div class="p-5">
            ...
            <section class="store-user">
                    <center><h3 style="text-transform:capitalize;color:#22b8d1">my payments<h3></center>
           ...
           <hr>
            ...
            <section class="product-area mt-4">
               <h5 class="thead" style="color:white;text-transform:capitalize;border-radius:5px;
                height:3rem;padding-top:11px">
                    payments report summary from {{ $fromDate }} to {{ $toDate }}</h5>
